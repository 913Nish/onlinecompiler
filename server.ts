...
// Resilient Gemini generator with cascade across compatible models
async function callGeminiWithCascade(
  ...
  // Try gemini-3.1-flash-lite first for high availability and low latency, followed by gemini-3.8-flash and gemini-flash-latest
  ...
  for (const model of candidateModels) {
    try {
      ...
      lastError = err;
      // On 503 (high demand), 504 (timeout), 429 (rate limit), cascade gracefully to the next model
...
app.post('/api/compile', async (req: Request, res: Response) => {
  try {
    ...
    if (!ai) {
      // In case GEMINI_API_KEY is not configured, inform client so client-side compiler engine handles it
      res.status(200).json({
        fallback: true,
...
// Real-time execution endpoint with STDIN support
app.post('/api/execute', async (req: Request, res: Response) => {
  ...
  // 1. Native execution for Python
  if (lang === 'python' || lang === 'python3' || lang === 'py') {
    const tempFile = path.join(os.tmpdir(), `nextleap_${Date.now()}_${Math.random().toString(36).slice(2)}.py`);
    try {
      fs.writeFileSync(tempFile, code, 'utf-8');

      const proc = spawn('/usr/bin/python3', [tempFile]);
      let stdout = '';
      let stderr = '';
  ...
  // 2. Native execution for Node / JavaScript
  if (lang === 'javascript' || lang === 'js' || lang === 'node') {
    const tempFile = path.join(os.tmpdir(), `nextleap_${Date.now()}_${Math.random().toString(36).slice(2)}.mjs`);
    try {
      fs.writeFileSync(tempFile, code, 'utf-8');

      const proc = spawn('/usr/local/bin/node', [tempFile]);
      let stdout = '';
      let stderr = '';
      let isTimedOut = false;
  ...
  // 3. Native execution for TypeScript
  if (lang === 'typescript' || lang === 'ts') {
    const tempFile = path.join(os.tmpdir(), `nextleap_${Date.now()}_${Math.random().toString(36).slice(2)}.ts`);
    try {
      fs.writeFileSync(tempFile, code, 'utf-8');
  ...
  // 4. Native compilation and execution for C (GCC)
  if (lang === 'c') {
    const fileId = `nextleap_${Date.now()}_${Math.random().toString(36).slice(2)}`;
    const srcFile = path.join(os.tmpdir(), `${fileId}.c`);
    const binFile = path.join(os.tmpdir(), `${fileId}.out`);

    try {
      fs.writeFileSync(srcFile, code, 'utf-8');

      // Compile with gcc
      const compileProc = spawn('gcc', ['-O2', srcFile, '-o', binFile, '-lm']);
      let compileErr = '';
  ...
  // 5. Native compilation and execution for C++ (G++)
  if (lang === 'cpp' || lang === 'c++' || lang === 'cc') {
    const fileId = `nextleap_${Date.now()}_${Math.random().toString(36).slice(2)}`;
    const srcFile = path.join(os.tmpdir(), `${fileId}.cpp`);
    const binFile = path.join(os.tmpdir(), `${fileId}.out`);
    ...
    try {
      ...
      // Compile with g++
      const compileProc = spawn('g++', ['-O2', '-std=c++17', srcFile, '-o', binFile, '-lm']);
      let compileErr = '';
  ...
  // 6. Native compilation and execution for Java (OpenJDK 17)
  if (lang === 'java') {
    const fileId = `exec_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
    const tempDir = path.join(os.tmpdir(), fileId);
    ...
    try {
      ...
      const compileProc = spawn('javac', ['-encoding', 'UTF-8', srcFile]);
      let compileErr = '';
      ...
      compileProc.on('close', (cCode) => {
        ...
        const runProc = spawn('java', ['-cp', tempDir, className]);
        let stdout = '';
        let stderr = '';
        let isTimedOut = false;
  ...
  // 7. Native compilation and execution for Rust (rustc)
  if (lang === 'rust' || lang === 'rs') {
    const fileId = `exec_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
    const srcFile = path.join(os.tmpdir(), `${fileId}.rs`);
    const binFile = path.join(os.tmpdir(), `${fileId}.bin`);

    try {
      fs.writeFileSync(srcFile, code, 'utf-8');

      const compileProc = spawn('rustc', ['-O', srcFile, '-o', binFile]);
      let compileErr = '';
  ...
  // 8. Native execution for Go (golang)
  if (lang === 'go' || lang === 'golang') {
    const fileId = `exec_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
    const srcFile = path.join(os.tmpdir(), `${fileId}.go`);

    try {
      fs.writeFileSync(srcFile, code, 'utf-8');

      const runProc = spawn('go', ['run', srcFile]);
      let stdout = '';
      let stderr = '';
      let isTimedOut = false;
  ...
  // 9. Native execution for PHP
  if (lang === 'php') {
    const fileId = `exec_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
    const srcFile = path.join(os.tmpdir(), `${fileId}.php`);

    try {
      fs.writeFileSync(srcFile, code, 'utf-8');

      const runProc = spawn('php', [srcFile]);
      let stdout = '';
      let stderr = '';
      let isTimedOut = false;
  ...
  // 10. Native execution for Bash / Shell scripts
  if (lang === 'bash' || lang === 'sh' || lang === 'shell') {
    const fileId = `exec_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
    const srcFile = path.join(os.tmpdir(), `${fileId}.sh`);

    try {
      fs.writeFileSync(srcFile, code, 'utf-8');

      const runProc = spawn('bash', [srcFile]);
      let stdout = '';
      let stderr = '';
      let isTimedOut = false;
  ...
  // 11. For other languages (Swift, Kotlin, Ruby, etc.): Execute with Gemini
  const ai = getGenAI();
  if (ai) {
    try {
      const response = await callGeminiWithCascade(ai, {
...
You MUST return ONLY a JSON object with this format:
{
  ...
  "memoryMb": 3.4
      ...
      // Fall through to deterministic syntax and simulated execution
    }
  }

  // 12. Smart AST & Output Extractor Fallback (never output canned generic messages)
  try {
    // Extract print/log outputs from common language patterns
    const printRegexes = [
      /print(?:ln)?!\s*\(\s*"([^"]*)"\s*(?:,\s*([^)]*))?\)/g,
  ...
  // Default execution output for unsupported languages
  res.json({
...
// Dark Web AI Tutor Endpoint (Comprehensive Analysis & Interactive Chat)
app.post('/api/ai-assist', async (req: Request, res: Response) => {
  ...
  // Helper for dynamic local fallback responses
  const generateFallbackResponse = () => {
    // 1. Runtime Error Diagnostic
    if (action === 'diagnose_runtime' || (runtimeContext && (runtimeContext.exitCode !== 0 || runtimeContext.stderr))) {
      ...
      return {
        ...
        content: `### 🚨 Execution Error Analysis
...
- **Next Step:** Would you like me to trace a specific line or test this against tricky edge cases like empty arrays or negative numbers?`,
    ...
    if (action === 'socratic') {
      return {
        ...
        content: `### 🧠 Think Through Your Solution
...
*Reply below with your thoughts and I'll validate your reasoning!*`,
    ...
    if (action === 'linebyline') {
      ...
      const breakdown = lines
        .map((l, i) => `**Line ${i + 1}** (\`${l.trim() || 'empty'}\`): Sets up variable state or control branch.`)
        .join('\n');
      return {
        action: 'explain',
...
For each, provide the input and expected output.`;
    ...
    const conversationSnippet = conversationHistory.length > 0
...
Return a JSON object:
{
  ...
  "metrics": {
    ...
    // Graceful fallback to curriculum analysis
    res.json(generateFallbackResponse());
  }
});
